...
研習當天上半場（09:00–10:25）由陳文盛帶關 1，學員只跟 Codex 講話，由它呼叫 `agy` 完成規劃、實作與複核；下半場（10:35–12:00）由游豐兆帶學生回饋、關 2 名單出席與成果整理，使用單一 Antigravity 桌面版（關 3–5 仍由游豐兆主筆）。上、下半場作品分別在 10:22、11:50 收件。早上現場必備的是 ChatGPT／Codex、Antigravity 桌面版與 `agy`。GitHub 帳號、Git 與 GitHub CLI 是會後讓 AI 做 `commit`／`push`／`clone`／`pull` 才用到：有最好，沒裝仍請來；GitHub Desktop 選配。
